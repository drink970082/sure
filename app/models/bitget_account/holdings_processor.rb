...
class BitgetAccount::HoldingsProcessor
  ...
  def process
    ...
    Rails.logger.error "BitgetAccount::HoldingsProcessor - error: #{e.message}"
    nil
  end
  ...
  private
    ...
    def raw_assets
      bitget_account.raw_payload&.dig("assets") || []
    end
    ...
    def process_asset(asset)
      ...
      import_adapter.import_holding(
        ...
        price: price,
        cost_basis: nil,
        external_id: "bitget_#{symbol}_#{source}_#{Date.current}",
        account_provider_id: bitget_account.account_provider&.id,
        source: "bitget",
        delete_future_holdings: false
      )
    rescue StandardError => e
      Rails.logger.error "BitgetAccount::HoldingsProcessor - failed asset symbol=#{symbol.presence || "unknown"}: #{e.message}"
    end
    ...
    def resolve_security(symbol)
      ...
      BitgetAccount::SecurityResolver.resolve(ticker, symbol)
    end

    def log_stale_rate(symbol, field, rate_date)
      Rails.logger.warn(
        "BitgetAccount::HoldingsProcessor - stale FX rate for #{field} symbol=#{symbol} rate_date=#{rate_date || "unknown"}"
      )
    end
end
